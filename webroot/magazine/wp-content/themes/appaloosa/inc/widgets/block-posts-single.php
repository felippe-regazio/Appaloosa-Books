...
class HitMag_Single_Category_Posts extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		<div class="hitmag-one-category hitmag-posts-grid">
			<?php 
			if ( $latest_posts -> have_posts() ) :				
				...
				while ( $latest_posts -> have_posts() ): $latest_posts -> the_post(); ?>
					...
					<!-- dont use the first 5 posts on slider -->					
					<?php if ( has_post_thumbnail() && $hmp_count <= 5 ){ 
						$hmp_count++; ?>
					<?php }else{ ?>
						<!-- 
							if has image the class is hms-post, if not, use
							hms-post-full
						-->
						<?php 
						...
						<div class="<?=$post_class?>">
							...
							<div class="hms-details">
								<?php 
									the_title( sprintf( '<h3 class="hms-title"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h3>' );
